...
public class HibernateCriteria {
    public static void main(String[] args) {
        ...
        clientes = em.createQuery(query).setParameter("nombre", "Sebas").getResultList();
        clientes.forEach(System.out::println);

        System.out.println("========== usando where like para buscar clientes por nombre ==========");
        query = criteria.createQuery(Cliente.class);
        from = query.from(Cliente.class);
        ParameterExpression<String> nombreParamLike = criteria.parameter(String.class, "nombreParamLike");

        query.select(from).where(criteria.like(criteria.upper(from.get("nombre")), criteria.upper(nombreParamLike)));
        clientes = em.createQuery(query).setParameter("nombreParamLike", "%s%")
                .getResultList();
        clientes.forEach(System.out::println);
        ...
        clientes = em.createQuery(query).setParameter("nombres", Arrays.asList("santi", "sebas", "ale")).getResultList();
        clientes.forEach(System.out::println);
        ...
        System.out.println("========== consulta de predicados de conjunción and y disyunción or ==========");
        query = criteria.createQuery(Cliente.class);
        from = query.from(Cliente.class);
        Predicate porNombre = criteria.equal(from.get("nombre"), "sebas");
        ...
        query = criteria.createQuery(Cliente.class);
        from = query.from(Cliente.class);
        query.select(from).orderBy(criteria.asc(from.get("apellido")), criteria.asc(from.get("nombre")));
        clientes = em.createQuery(query).getResultList();
        clientes.forEach(System.out::println);
        ...
        Cliente cliente = em.createQuery(query).setParameter("id", 1L)
                .getSingleResult();
        ...
        from = queryObject.from(Cliente.class);
        queryObject.multiselect(from.get("id"), from.get("nombre"), from.get("apellido")).where(criteria.like(from.get("nombre"), "%s%"));
        ...
        System.out.println("========== consulta para contar registros con count ==========");
        ...
        System.out.println("========== sumar datos de algún campo de la tabla con sum ==========");
        ...
        System.out.println("========== consulta con el máximo id con max ==========");
        queryLong = criteria.createQuery(Long.class);
        from = queryLong.from(Cliente.class);
        ...
        System.out.println("========== consulta con el minimo id con min ==========");
        queryLong = criteria.createQuery(Long.class);
        from = queryLong.from(Cliente.class);
